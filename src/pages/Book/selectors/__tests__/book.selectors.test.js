...
describe('books selectors', () => {
  ...
  it('returns status is loading from the state', () => {
    ...
    expect(selectors.bookFetchIsStatusInProgress(store.getState())).toEqual(
      isStatusLoading
    );
  });
  ...
  it('returns create status from the state', () => {
    ...
    expect(selectors.bookCreateStatusSelector(store.getState())).toEqual(
      status
    );
  });
  ...
  it('returns create status is loading from the state', () => {
    ...
    const isStatusLoading = status === ACTION_STATUS.IN_PROGRESS;
    expect(selectors.bookCreateIsStatusInProgress(store.getState())).toEqual(
      isStatusLoading
    );
  });
  ...
  it('returns update status from the state', () => {
    ...
    expect(selectors.bookUpdateStatusSelector(store.getState())).toEqual(
      status
    );
  });
  ...
  it('returns update status is loading from the state', () => {
    ...
    const isStatusLoading = status === ACTION_STATUS.IN_PROGRESS;
    expect(selectors.bookUpdateIsStatusInProgress(store.getState())).toEqual(
      isStatusLoading
    );
  });
  ...
  it('returns delete status from the state', () => {
    ...
    expect(selectors.bookDeleteStatusSelector(store.getState())).toEqual(
      status
    );
  });
  ...
  it('returns delete status is loading from the state', () => {
    ...
    const isStatusLoading = status === ACTION_STATUS.IN_PROGRESS;
    expect(selectors.bookDeleteIsStatusInProgress(store.getState())).toEqual(
      isStatusLoading
    );
  });
});
