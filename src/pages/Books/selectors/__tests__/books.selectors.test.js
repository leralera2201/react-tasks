...
describe('books selectors', () => {
  ...
  it('returns item status from the state', () => {
    ...
    expect(selectors.booksFetchStatusSelector(store.getState())).toEqual(
      status
    );
  });
  ...
  it('returns current books from the state', () => {
    ...
    expect(selectors.booksFetchCurrentItemsSelector(store.getState())).toEqual(
      currentBooks
    );
  });
  ...
  it('returns current page from the state', () => {
    ...
    expect(selectors.booksFetchCurrentPageSelector(store.getState())).toEqual(
      currentPage
    );
  });
  ...
  it('returns current books per page from the state', () => {
    ...
    expect(
      selectors.booksFetchBooksPerPagePageSelector(store.getState())
    ).toEqual(booksPerPage);
  });
  ...
  it('returns is status loading from the state', () => {
    ...
    expect(selectors.booksFetchIsStatusInProgress(store.getState())).toEqual(
      isStatusLoading
    );
  });
});
